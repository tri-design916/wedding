...
import { ArrowDown, ArrowLeft, ArrowRight, CalendarBlank, CaretDown, ChatCircle, Check, Copy, MapPin, Phone, ShareNetwork, X } from "@phosphor-icons/react";

const scenes = Array.from({ length: 6 }, (_, i) => `/assets/scenes/scene-${i + 1}.png`);
const gallery = ["DSC_4728","DSC_4853","DSC_5023","DSC_5245","DSC_5462","DSC_5526","DSC_5681","DSC_5739","DSC_5936","DSC_6207","DSC_6261","DJI_0663"].map(name => ({ name, src: `/assets/photos/${name}.jpg` }));
...
function Gallery(){const [active,setActive]=useState(null);useEffect(()=>{if(active===null)return;const onKey=e=>{if(e.key==="Escape")setActive(null);if(e.key==="ArrowRight")setActive((active+1)%gallery.length);if(e.key==="ArrowLeft")setActive((active-1+gallery.length)%gallery.length)};document.body.classList.add("modal-open");addEventListener("keydown",onKey);return()=>{document.body.classList.remove("modal-open");removeEventListener("keydown",onKey)}},[active]);const move=d=>setActive((active+d+gallery.length)%gallery.length);return <>{<div className="gallery-grid">{gallery.map((image,i)=><button key={image.name} onClick={()=>setActive(i)} aria-label={`${i+1}번째 사진 크게 보기`}><img src={image.src} alt={`웨딩 스냅 ${i+1}`} loading="lazy"/><span>{String(i+1).padStart(2,"0")}</span></button>)}</div>}{active!==null&&<div className="lightbox" role="dialog" aria-modal="true" aria-label="사진 크게 보기"><button className="lightbox-close" onClick={()=>setActive(null)} aria-label="닫기"><X size={24}/></button><button className="lightbox-nav prev" onClick={()=>move(-1)} aria-label="이전 사진"><ArrowLeft size={25}/></button><img src={gallery[active].src} alt={`${active+1}번째 웨딩 스냅`}/><button className="lightbox-nav next" onClick={()=>move(1)} aria-label="다음 사진"><ArrowRight size={25}/></button><p>{active+1} / {gallery.length}</p></div>}</>}
...
function App(){
  ...
  return <main>
    ...
    <section className="date-section section-pad"><SectionTitle eyebrow="THE DATE">시월의 마지막 토요일</SectionTitle><p className="date-lead">2026년 10월 31일 토요일<br/><strong>오후 3시</strong></p><Calendar/><a className="calendar-link" href="https://calendar.google.com/calendar/render?action=TEMPLATE&text=%EC%9D%B4%EC%9E%AC%EB%AA%A8%20%EC%84%9C%ED%98%84%EC%95%84%20%EA%B2%B0%ED%98%BC%EC%8B%9D&dates=20261031T060000Z/20261031T080000Z&location=%EA%B4%91%EB%AA%85%EC%95%84%EC%9D%B4%EB%B2%A1%EC%8A%A4%EC%BB%A8%EB%B2%A4%EC%85%98" target="_blank" rel="noreferrer"><CalendarBlank size={18}/> 캘린더에 저장</a></section>
    <section className="gallery-section section-pad"><SectionTitle eyebrow="OUR MOMENTS">빛나는 장면들</SectionTitle><p className="section-note">사진을 누르면 크게 볼 수 있어요.</p><Gallery/></section>
    <section className="location-section section-pad"><SectionTitle eyebrow="LOCATION">오시는 길</SectionTitle><div className="venue-name"><strong>광명아이벡스컨벤션</strong><span>AK플라자 광명점 5층</span></div><p className="address">경기 광명시 양지로 17</p><div className="map-actions"><a href="https://map.naver.com/p/search/%EA%B4%91%EB%AA%85%EC%95%84%EC%9D%B4%EB%B2%A1%EC%8A%A4%EC%BB%A8%EB%B2%A4%EC%85%98" target="_blank" rel="noreferrer"><MapPin size={18}/>네이버 지도</a><a href="https://map.kakao.com/?q=%EA%B4%91%EB%AA%85%EC%95%84%EC%9D%B4%EB%B2%A1%EC%8A%A4%EC%BB%A8%EB%B2%A4%EC%85%98" target="_blank" rel="noreferrer"><MapPin size={18}/>카카오맵</a></div><div className="transit-note"><small>TRANSPORTATION</small><p>교통 및 주차 안내는 최종 정보 확인 후 업데이트될 예정입니다.</p></div></section>
    ...
    <section className="closing-section"><img src="/assets/photos/DSC_6207.jpg" alt="함께 웃는 이재모와 서현아" loading="lazy"/><div className="closing-card"><p>저희의 소중한 날을<br/>함께해 주셔서 감사합니다.</p><span>보내주신 따뜻한 마음을 오래도록 간직하며<br/>서로 아끼고 사랑하며 잘 살아가겠습니다.</span><strong>이재모 · 서현아 드림</strong></div></section>
